...
本轮将曲线编辑器的选中态改为独立的电紫色语义，深色与浅色主题分别使用 `#B69CFF`、`#6D28D9`，绿色继续只表示当前音量。控制点采用 `17dp` 径向渐隐光晕和 `7dp` 细环，原控制点本体不变；线段采用 `17/10/5.5dp` 三层荧光轨道，再覆盖原 `3dp` 曲线本体，因此选中不会改变对象自身颜色。顶部与右侧绘图区留白同步增至 `18dp`，避免端点光晕裁切；绿色当前音量交点最后绘制，和选中态重合时仍保持清楚。
...
- API 37 AVD 上分别截取浅色、深色的选中控制点与选中线段画面并逐张目视核验：整屏比例下选中对象可立即识别，荧光柔和且边界明确，无端点裁切；未选对象、曲线本体和绿色当前音量线没有视觉回归；
...
- 设备测试中的绘图区几何参数已同步为新的顶部/右侧 `18dp` 留白，双轴拖动仍强制吸附到整数坐标；
