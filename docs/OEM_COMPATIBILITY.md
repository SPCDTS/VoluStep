...
| 平台/路由 | 可能差异 | 当前策略 |
|---|---|---|
| 小米 MIUI / HyperOS | 媒体可为 0–150；存在“调节媒体音量”权限和后台限制 | 运行时读取范围；双阶段回读；主界面“设备”折叠区提供公开应用详情入口与保守排障提示 |
...
Xiaomi 15 / HyperOS / Android 16（API 36）的扬声器媒体范围为 `0..150`。在线性 1% 短按步长、按 index 量化配置下，实体音量键在 Activity 前台、桌面后台和亮屏锁屏时均完成 `62 → 63 → 62`；熄屏 Doze 且没有媒体播放时，按键没有触发应用或系统音量写入，音量保持 `62`。目标无障碍服务可与设备原有服务同时保持 Bound，FGS 运行状态正常；当前 API 33+ 构建不请求通知权限，因此普通通知抽屉不保留常驻通知，用户可从系统“运行中的应用”入口查看和停止。
...
当前直接分发构建另采用与李跳跳运行形态相近的实验策略：主界面任务始终从最近任务排除，避免用户上划卡片触发 HyperOS `SwipeUpClean → FORCE STOP`；无障碍服务连接期间持有透明不可交互的 1×1 `TYPE_ACCESSIBILITY_OVERLAY`。这两项都只使用公开 API，也不需要悬浮窗权限，但 AOSP 不承诺它们提高进程优先级。它们不能恢复授权、抵抗真正的 force-stop 或保证息屏投递；应分别在每个 OEM 上记录 task、服务、进程与按键时序，不能仅凭窗口存在判定兼容性通过。
