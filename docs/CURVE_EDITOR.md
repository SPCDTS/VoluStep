...
1. 顶部用紧凑步进器设置 `K`，提供减一、整数输入和加一；键盘输入合法新值时显示行内确认。修改次数后保持原曲线外形，再把采样结果投影到合法整数序列。
2. 主画布固定均匀显示所有状态点。点击选择一点；纵向拖动修改 index，横向经过其他点时形成画笔式连续编辑。拖过相邻点时采用最小推挤，而不是把当前点卡在邻点前一格，因此密集曲线仍然可画。
3. 默认选中点摘要显示 `第 i/K 步`、当前 index 与 `Δindex`，并常驻 `-1` / `+1`。状态 Slider 和精确整数输入收在“精确编辑”中，端点可选中查看但不可改写。
...
- 当横轴列间距小于 `8dp` 时，只绘制端点和当前选中点，保留折线与 `Δindex` 条带，避免高 `K` 下圆点重叠成视觉噪声。
